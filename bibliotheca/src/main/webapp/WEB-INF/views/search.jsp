...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
	    ...
	    <c:choose>
	    	<c:when test="${fn:length(page.books) == 0}">
			    <div class="alert alert-danger top-alert">
			      <strong>No Results Found for "${search}".</strong>
			    </div>
		    </c:when>
		    <c:otherwise>
	    ...
	    <c:url var="firstUrl" value="?s=${search}&page=0" />
	    <c:url var="lastUrl" value="?s=${search}&page=${page.total}" />
	    <ul class="pagination pagination-lg">
	        ...
	        <c:forEach var="i" begin="${page.begin}" end="${page.end}">
	            <c:url var="pageUrl" value="?s=${search}&page=${i}" />
	            <c:choose>
